...
export const createFhevmInstance = async (parameters: {
  ...
  // Dynamic config based on chainId
  let config: FhevmInstanceConfig;
  let aclAddress: `0x${string}`;

  if (chainId === 11155111) {
    // Sepolia
    aclAddress = relayerSDK.SepoliaConfig.aclContractAddress as `0x${string}`;
    config = {
      ...relayerSDK.SepoliaConfig,
      network: providerOrUrl,
    };
  } else if (chainId === 31337) {
    // Localhost/Hardhat - use SepoliaConfig as fallback
    aclAddress = relayerSDK.SepoliaConfig.aclContractAddress as `0x${string}`;
    config = {
      ...relayerSDK.SepoliaConfig,
      network: providerOrUrl,
    ...
    throw new Error(`Unsupported chainId: ${chainId}. Only Sepolia (11155111) and Hardhat (31337) are supported.`);
  }
