...
const CONTRACT_NAME = "ConfidentialTokenFixed";
...
// Skip ABI comparison - use Sepolia ABI if available, otherwise use localhost
let contractAbi;
if (deploySepolia && deploySepolia.address !== "0x0000000000000000000000000000000000000000") {
  contractAbi = JSON.stringify({ abi: deploySepolia.abi }, null, 2);
  console.log("Using Sepolia ABI");
} else {
  contractAbi = JSON.stringify({ abi: deployLocalhost.abi }, null, 2);
  console.log("Using localhost ABI");
}
